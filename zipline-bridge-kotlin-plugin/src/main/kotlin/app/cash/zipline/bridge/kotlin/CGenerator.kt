    ...
    for (field in fields) {
      ...
      when {
        ...
        field.isObjectType -> {
          ...
          appendLine("        JniBridgeDispatch *disp_${field.name} = (JniBridgeDispatch *)(intptr_t)JS_VALUE_GET_FLOAT64(disp_val_${field.name});")
...
internal fun emitGeneralArrayLoop(
  ...
  sb.appendLine("                        JniBridgeDispatch *disp = (JniBridgeDispatch *)(intptr_t)JS_VALUE_GET_FLOAT64(disp_val);")
...
internal fun emitAnyFieldExtraction(
  ...
  sb.appendLine("        // Any? field — dispatch on JS value tag")
  sb.appendLine("        {")
  sb.appendLine("            int any_tag_${field.name} = JS_VALUE_GET_NORM_TAG(js_${field.name});")
  // Always use Double for JS numbers — JS doesn't distinguish Int from Float.
  sb.appendLine("            if (any_tag_${field.name} == JS_TAG_INT) {")
  sb.appendLine("                $javaVar = (*env)->NewObject(env, _any_boxed_Double_cls, _any_boxed_Double_ctor, (jdouble)(jint)JS_VALUE_GET_INT(js_${field.name}));")
  sb.appendLine("            } else if (any_tag_${field.name} == JS_TAG_FLOAT64) {")
  sb.appendLine("                $javaVar = (*env)->NewObject(env, _any_boxed_Double_cls, _any_boxed_Double_ctor, (jdouble)JS_VALUE_GET_FLOAT64(js_${field.name}));")
  sb.appendLine("            } else if (any_tag_${field.name} == JS_TAG_BOOL) {")
  sb.appendLine("                $javaVar = (*env)->NewObject(env, _any_boxed_Boolean_cls, _any_boxed_Boolean_ctor, (jboolean)JS_VALUE_GET_BOOL(js_${field.name}));")
  sb.appendLine("            } else if (any_tag_${field.name} == JS_TAG_STRING) {")
  sb.appendLine("                const char *any_str_${field.name} = JS_ToCString(ctx, js_${field.name});")
  sb.appendLine("                $javaVar = (*env)->NewStringUTF(env, any_str_${field.name});")
  sb.appendLine("                JS_FreeCString(ctx, any_str_${field.name});")
  sb.appendLine("            } else if (any_tag_${field.name} == JS_TAG_OBJECT) {")
  sb.appendLine("                JSValue any_disp_${field.name} = JS_GetPropertyStr(ctx, js_${field.name}, \"bridge_dispatch\");")
  sb.appendLine("                if (JS_IsUndefined(any_disp_${field.name})) {")
  sb.appendLine("#ifdef __ANDROID__")
  sb.appendLine("                    const char *adn_${field.name} = \"(unknown)\";")
  sb.appendLine("                    JSValue adc_${field.name} = JS_GetPropertyStr(ctx, js_${field.name}, \"constructor\");")
  sb.appendLine("                    if (!JS_IsUndefined(adc_${field.name}) && !JS_IsNull(adc_${field.name})) {")
  sb.appendLine("                        JSValue adnc_${field.name} = JS_GetPropertyStr(ctx, adc_${field.name}, \"name\");")
  sb.appendLine("                        adn_${field.name} = JS_ToCString(ctx, adnc_${field.name});")
  sb.appendLine("                        JS_FreeValue(ctx, adnc_${field.name});")
  sb.appendLine("                    }")
  sb.appendLine("                    __android_log_print(ANDROID_LOG_ERROR, \"BRIDGE\", \"Any? field ${field.name} in $jniClassName: bridge_dispatch not found (ctor=%s)\", adn_${field.name});")
  sb.appendLine("                    if (strcmp(adn_${field.name}, \"(unknown)\") != 0) JS_FreeCString(ctx, adn_${field.name});")
  sb.appendLine("                    JS_FreeValue(ctx, adc_${field.name});")
  sb.appendLine("#endif")
  sb.appendLine("                    JS_FreeValue(ctx, any_disp_${field.name});")
  sb.appendLine("                    JS_FreeValue(ctx, js_${field.name});")
  sb.appendLine("                    return NULL;")
  sb.appendLine("                }")
  sb.appendLine("                JniBridgeDispatch *any_disp_p_${field.name} = (JniBridgeDispatch *)(intptr_t)JS_VALUE_GET_FLOAT64(any_disp_${field.name});")
  sb.appendLine("                $javaVar = any_disp_p_${field.name}->toJavaObject(env, ctx, &js_${field.name});")
  sb.appendLine("                JS_FreeValue(ctx, any_disp_${field.name});")
  sb.appendLine("            } else {")
  sb.appendLine("#ifdef __ANDROID__")
  sb.appendLine("                __android_log_print(ANDROID_LOG_ERROR, \"BRIDGE\", \"Any? field ${field.name}: unexpected JS tag %d\", any_tag_${field.name});")
  sb.appendLine("#endif")
  sb.appendLine("                JS_FreeValue(ctx, js_${field.name});")
  sb.appendLine("                return NULL;")
  sb.appendLine("            }")
